getgenv().BetaKey = "CatRevival"
getgenv().CheckKey = "CatRevival"

if getgenv().CheckKey == BetaKey then
--Loading Screen
local screen = Instance.new("ScreenGui")
local frame = game:GetService("Players").LocalPlayer.PlayerGui.Transition.Background:Clone()
local text = game:GetService("Players").LocalPlayer.PlayerGui.Transition.Hint:Clone()
local textOGPOS = text.Position
local img = game:GetService("Players").LocalPlayer.PlayerGui.Transition.GifFrame:Clone()
local cycle = coroutine.create(function()
    while true do
        for i=1, 5 do
            for k=1, 4 do
                img.GifImage.Position -= UDim2.fromScale(1, 0)
                for p=1, 5 do game:GetService("RunService").Heartbeat:Wait() end
            end
            if img.GifImage.Position.Y.Scale ~= -4 then
                img.GifImage.Position -= UDim2.fromScale(0, 1)
                img.GifImage.Position += UDim2.fromScale(4, 0)
            end
            for p=1, 5 do game:GetService("RunService").Heartbeat:Wait() end
        end
        img.GifImage.Position = UDim2.fromScale(0, 0)
    end
end)
...
text.HintLabel.TextTransparency = 1
text.HintLabel.Text = "Cat Hub Revival"
    
img.Parent = screen
img.GifImage.Image = "rbxassetid://12521177072"
img.GifImage.ImageTransparency = 1
img.GifImage.Size = UDim2.fromScale(6, 5)
img.GifImage.Position = UDim2.fromScale(0, 0)
    
local tweens = {
    {
        gonwait = true,
        tween = game:GetService("TweenService"):Create(frame, TweenInfo.new(1.5), {Size = UDim2.new(0, 5000, 0, 5000)})
    },
    {
        gonwait = false,
        tween = game:GetService("TweenService"):Create(text.HintLabel, TweenInfo.new(1), {TextTransparency = 0})
    },
    {
        gonwait = false,
        tween = game:GetService("TweenService"):Create(text, TweenInfo.new(1, Enum.EasingStyle.Sine, Enum.EasingDirection.Out), {Position = textOGPOS})
    },
    {
        gonwait = false,
        tween = game:GetService("TweenService"):Create(img.GifImage, TweenInfo.new(1), {ImageTransparency = 0})
    }
}
...
local reversetweens = {
    {
        gonwait = false,
        tween = game:GetService("TweenService"):Create(img.GifImage, TweenInfo.new(1), {ImageTransparency = 1})
    },
    {
        gonwait = false,
        tween = game:GetService("TweenService"):Create(text.HintLabel, TweenInfo.new(1), {TextTransparency = 1})
    },
    {
        gonwait = false,
        tween = game:GetService("TweenService"):Create(text, TweenInfo.new(1, Enum.EasingStyle.Sine, Enum.EasingDirection.In), {Position = textOGPOS + UDim2.new(0, 0, 0, 40)})
    },
    {
        gonwait = true,
        tween = game:GetService("TweenService"):Create(frame, TweenInfo.new(1), {Size = UDim2.new(0, 0, 0, 0)})
    }
}
    
coroutine.resume(cycle) --start gif
...
wait(5)
coroutine.close(cycle) --stop gif
for i,v in pairs(reversetweens) do
    v.tween:Play()
    if v.gonwait then
        v.tween.Completed:Wait()
    end
...
ObbyTab:AddButton({
	...
	Callback = function()
        if SelectObby == "Spawn Obby" or SelectObby == "SpawnObby" then
        ...
        elseif SelectObby == "Jungle Obby" or SelectObby == "JungleObby" then
        ...
        elseif SelectObby == "Atlantis Obby" or SelectObby == "Atlantis" then
wait(2)
...
game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(2105.671, 98.214, -2304.623)
        elseif SelectObby == "Pyramid Obby" or SelectObby == "PyramidObby" then
        ...
        elseif SelectObby == "Ice Obby" or SelectObby == "IceObby" then
        ...
        elseif SelectObby == "Cart Ride into Preston" or SelectObby == "Minecraft" then
...
local DigTabe = Window:MakeTab({
    Name = "Fishing/Digsite",
    Icon = "rbxassetid://16717409986",
    PremiumOnly = false
})

local Section = DigTabe:AddSection({
...
DigTabe:AddDropdown({
	...
	Default = "",
	Options = {"Fishing", "AdvancedFishing"},
	Callback = function(Value)
        getgenv().SelectFish = Value
    end    
})
...
DigTabe:AddToggle({
    ...
    if AutoFishing == true then
        while AutoFishing == true and task.wait() do
        local x = math.random(10, 20)
        local z = math.random(10, 20)

        local argsCast = {
            [1] = getgenv().SelectFish,
            [2] = "RequestCast",
...
local Section = DigTabe:AddSection({
    Name = "Auto Digsite Farm"
})

DigTabe:AddDropdown({
	Name = "Select Digsite Area",
	Default = "",
	Options = {"Digsite", "AdvancedDigsite"},
	Callback = function(Value)
         getgenv().SelectDigsite = Value
    end    
})

local Section = DigTabe:AddSection({
    Name = "Settings Digsite"
})

--[[DigTabe:AddDropdown({
	Name = "Select Digsite Aura",
	Default = "",
	Options = {"1x1", "2x2", "3x3"},
	Callback = function(Value)
         getgenv().DigAura = Value
    if DigAura == "1x1" then
        DigAura = 5
    elseif DigAura == "2x2" then
        DigAura = 10
    elseif DigAura == "3x3" then
        DigAura = 15
    end
    end    
})]]--
...
function AutoEsp()
if AutoEspx == true then
while AutoEspx == true and task.wait() do
...
function AutoChestTp()
if AutoChestss == true then
while AutoChestss == true and task.wait() do
if Workspace.__THINGS.__INSTANCE_CONTAINER.Active.Digsite.Important.ActiveChests:FindFirstChild("Animated") then
DigsiteMap = Workspace.__THINGS.__INSTANCE_CONTAINER.Active.Digsite.Important.ActiveChests.Animated.Top
game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = DigsiteMap.CFrame
else
wait(0.9)
end
end
...
function AutoChestTp2()
if AutoChestss == true then
while AutoChestss == true and task.wait() do
if Workspace.__THINGS.__INSTANCE_CONTAINER.Active.AdvancedDigsite.Important.ActiveChests:FindFirstChild("Animated") then
DigsiteMap = Workspace.__THINGS.__INSTANCE_CONTAINER.Active.AdvancedDigsite.Important.ActiveChests.Animated.Top
game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = DigsiteMap.CFrame
else
wait(0.9)
end
...
function AutoDigTest()
    while AutoDigsite == true do
...
DigTabe:AddToggle({
    ...
    Default = false,
    Callback = function(Value)
    getgenv().AutoDigsite = Value
if SelectDigsite == "Digsite" then
AutoDigTest()
elseif SelectDigsite == "AdvancedDigsite" then
AutoDigTest2()
end
end
})
...
DigTabe:AddToggle({
    Name = "Auto Teleport To Chests",
    Default = false,
    Callback = function(Value)
getgenv().AutoChestss = Value
if SelectDigsite == "Digsite" then
AutoChestTp()
elseif SelectDigsite == "AdvancedDigsite" then
AutoChestTp2()
...
local Section = GardenTab:AddSection({
    Name = "Auto Flower"
})

GardenTab:AddDropdown({
	Name = "Select Plant For Seed",
	Default = "",
	Options = {"Coin", "Potion", "Diamond", "Gift"},
	Callback = function(Value)
		getgenv().PlantSel = Value
	end    
})
...
GardenTab:AddToggle({
	...
	Callback = function(Value)
        ...
        for i=1,10 do
local args = {[1] = "FlowerGarden",[2] = "WaterSeed",[3] = i}
...
GardenTab:AddToggle({
	...
	Callback = function(Value)
        ...
        if MerchBuy == true then
            while MerchBuy == true and task.wait() do
                for i=1,10 do
    local args = {[1] = "FlowerGarden",[2] = "PlantSeed",[3] = i,[4] = PlantSel}
    game:GetService("ReplicatedStorage").Network.Instancing_InvokeCustomFromClient:InvokeServer(unpack(args))
        end
    end
        end
...
GardenTab:AddToggle({
	...
	Callback = function(Value)
        ...
        if MerchBuy == true then
            while MerchBuy == true and task.wait() do
                for i=1,10 do
                local args = {[1] = "FlowerGarden",[2] = "ClaimPlant",[3] = i}
...
elseif getgenv().CheckKey ~= BetaKey then
